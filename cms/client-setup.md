...
1. Copy the `client-template/` folder to `your-client/`
2. Create `cms/clients/your-client/_defaults.md` with `access: "client:your-client"`
3. Add client logos to `assets/images/logos/`
4. Customise `your-client/theme.css`
...
Copy the `client-template/` folder and rename it to the client name. Use lowercase, hyphenated names.

```
cp -r client-template/ your-client/
```
...
```
your-client/
  index.html      — Client homepage (auto-generated by docgen)
...
Create `cms/clients/your-client/_defaults.md`:
...
This ensures all pages generated for this client are scoped to their access level. You can optionally create a `cms/clients/your-client/welcome.md` for a welcome doc page.
...
```bash
node assets/js/svg-clean.js --strip-comments --size -o assets/images/logos/your-client.svg <<'SVGEOF'
...
3. Edit `acme/brand-book.html` — replace `[Client Name]` with "Acme" and update `data-access` to `client:acme`
4. Edit `acme/theme.css` — uncomment and set brand colours and fonts
5. Create `cms/clients/acme/_defaults.md`:
   ```yaml
   ---
   access: "client:acme"
   ---
   ```
6. Create `cms/clients/acme/welcome.md` (copy from client-template)
